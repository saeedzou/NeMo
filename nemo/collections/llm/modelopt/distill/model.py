...
class _DistillationLossReduction(MaskedTokenLossReduction):
    ...
    def forward(self, batch: Dict[str, Tensor], forward_out: Tensor) -> Tuple[Tensor, Dict[str, Tensor]]:
        ...
        lm_loss = average_losses_across_data_parallel_group([lm_loss])
        ...
        report = {
            "avg": average_losses_across_data_parallel_group([losses["kd_loss"]]),
            "logits distillation loss": average_losses_across_data_parallel_group([losses["logits_loss"]]),
            "intermediate distillation loss": average_losses_across_data_parallel_group([losses["intermediate_loss"]]),
        }
    ...
    def _masked_token_loss(self, loss_output: Tensor, mask: Tensor, num_valid_tokens_in_ub: Optional[int] = None):
        ...
        if self._cp_size > 1:
            if num_valid_tokens_in_ub is None:
                num_valid_tokens_in_ub = loss_mask_sum
