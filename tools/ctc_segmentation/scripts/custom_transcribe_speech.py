...
def load_model_and_processor(cfg: TranscriptionConfig):
    if cfg.model_type == "hezar":
        ...
        processor = None
    elif cfg.model_type == 'whisper_faim':
        processor = WhisperProcessor.from_pretrained('/media/eri-4090/HDD/Saman/Youtube Dataset/models/whisper/peyghan/processor', language='fa', task='transcribe')
        model = WhisperForConditionalGeneration.from_pretrained(cfg.model_name, use_cache=False)
...
def process_audio_files_batch(batch_files, processor, model_type):
    ...
    for audio_file in batch_files:
        ...
        if model_type in ["whisper", "whisper_faim"]:
...
def transcribe_audio(cfg: TranscriptionConfig, model, processor, device, model_type):
    ...
    if cfg.model_type in "whisper" and cfg.langid=='fa':
    ...
    for i in tqdm(range(0, len(filepaths), cfg.batch_size)):
        ...
        audio_input = None
        if model_type in ['wav2vec2', 'whisper', 'whisper_faim']:
        ...
        with torch.no_grad():
            ...
            elif model_type in ["whisper", "whisper_faim"]:
...
def main(cfg: TranscriptionConfig):
    ...
    elif 'peyghan' in cfg.model_name.lower():
        cfg.model_type = 'whisper_faim'
    elif 'hezar' in cfg.model_name.lower():
        cfg.model_type = 'hezar'
    elif 'whisper' in cfg.model_name.lower():
        cfg.model_type = 'whisper'
    else:
        raise ValueError(f'model should be whisper/wav2vec2/hezar/whisper')
